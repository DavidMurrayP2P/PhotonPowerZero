...
uint8_t fit_8_bits(uint16_t battery_voltage) {
        
    // Ensure the result value fits into a uint8_t
    uint8_t result;
    if (battery_voltage > UINT8_MAX) {
        result = UINT8_MAX;  // If overflow, set to the maximum value of uint8_t
    } else {
        result = (uint8_t)battery_voltage;
    }
...
int main( void ) {
  ...
  while(1) {
    ...
    if (is_Pi_Power_On()) {
        ...
        if (expert_mode == true) {
            if (is_Pi_Booted()) {
                if (is_Pi_ready_for_data()) {
                    PORTA |= (1 << ACTIVE);
                    shifted_voltage = fit_8_bits(battery_voltage);
        ...
        if ((battery_voltage*2) > 375) {
            turn_on_pi();
            }
        else if ((battery_voltage*2) < 345) {
            graceful_shutdown();
            turn_off_pi();    
            } 
        else {
